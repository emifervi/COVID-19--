...
class QuadrupleList(CovidListener):
    ...
    def parseArithmeticAsQuad(self, op1, op2):
        if len(self.operator_stack) == 0:
            return

        if self.operator_stack[-1] == op1 or self.operator_stack[-1] == op2:
            r_operand, r_type = self.operand_stack.pop()
            l_operand, l_type = self.operand_stack.pop()
            operator = self.operator_stack.pop()

            result_type = semantic_cube[r_type][l_type][operator]
    ...
    def exitFact_terms(self, ctx):
        self.parseArithmeticAsQuad(Operator.SUM, Operator.SUB)

    def exitOperand(self, ctx):
        self.parseArithmeticAsQuad(Operator.MULT, Operator.DIV)
